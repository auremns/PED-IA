...
CORS(PedIA, supports_credentials=True, origins=["https://ped-ia-front-76b329445fbd.herokuapp.com"]) 
PedIA.secret_key = 'laregulpourlesnuls'
...
def start_encounter():
    ...
    session[encounter_id] = {'clicked_classes': [], 
                             'parent_classes': {},
                             'displayed_classes': []}
...
def remove_displayed_class():
    ...
    if class_name in displayed_classes:
        ...
        #print(f"displayed classes : {displayed_classes}")
        return jsonify({"message": f"Removed {class_name} and related classes from displayed classes", "displayed_classes": list(displayed_classes)})
    else:
...
def get_motives():
    try:
        ...
        if motif_appel:
            subclasses = [subclass.label.first() if subclass.label else subclass.name for subclass in motif_appel.subclasses()]
...
def get_classes_after_faitRechercher():
    ...
    try:
        for element in cls.is_a:
            element, related_classes, displayed_classes = process_class_element(element, related_classes, displayed_classes)
...
def handle_context(element, clicked_classes):
    if isinstance(element, And):
        results = all(handle_context(sub_elem, clicked_classes) for sub_elem in element.Classes if not is_decision(sub_elem))
        print(f"Evaluating AND: {results}")
        return results
    elif isinstance(element, Or):
        results = any(handle_context(sub_elem, clicked_classes) for sub_elem in element.Classes if not is_decision(sub_elem))
            ...
            if parent_class_name:
                ...
                for element in parent_cls.is_a:
                    if isinstance(element, (And, Or, Restriction)):
                        ...
                        if result:
                            decision_list = [res for res in element.Classes if isinstance(res, Restriction) and res.property == onto.aPourDecisionRegulation]
                            if decision_list:
